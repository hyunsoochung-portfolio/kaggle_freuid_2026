...
class RecaptureAugWrapper(Dataset):
    ...
    NOTE: empirically this (and AnalogDoubleDataset's analog-double) both
    regressed public LB score in our own A/B tests, matching the
    feat/data-attention branch's own finding (dinov2_analog 0.115 vs
    dinov2_v1 0.0686 -- analog-double dropped). Kept for reference / possible
    future re-tuning, not used by any active config.
