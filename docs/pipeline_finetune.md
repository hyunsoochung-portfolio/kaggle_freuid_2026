...
| Attribution maps only partially localize tamper regions | An occlusion-sensitivity audit against 200 synthetic tampers finds the true edited region as the top-scoring pixel ~27.5% of the time (vs. a ~5.2% random baseline) — real, above-chance evidence of content-based reasoning, but not precise, reliable localization on most individual samples; see `reports/analysis_v0/attn/README.md` |
| Grad-CAM underperforms occlusion sensitivity sharply on this architecture | Likely a known limitation of adapting CNN-native class-activation methods to a Transformer's global self-attention, not evidence about the model's true reasoning — trust occlusion sensitivity over Grad-CAM for this model |
